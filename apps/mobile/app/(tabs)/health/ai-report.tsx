...
const STATUS_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  stable: { bg: '#dcfce7', text: '#166534', label: '穩定' },
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {report && (
        <View style={styles.resultCard}>
          {/* Status badge */}
          {(() => {
            const sc = STATUS_COLORS[report.status_label] ?? STATUS_COLORS.stable;
            return (
      ...
      {chatResult && (
        <View style={styles.resultCard}>
          {chatResult.result.explanation && (
            <Text style={styles.resultSummary}>{chatResult.result.explanation as string}</Text>
          )}
          {chatResult.result.message && (
            <Text style={styles.resultSummary}>{chatResult.result.message as string}</Text>
          )}
          {chatResult.result.key_points && (
            <View style={styles.resultSection}>
      ...
      {history.length > 0 && (
        <View style={styles.historySection}>
          ...
          {historyLoading ? (
            ...
            history.map((h) => {
              const sc = STATUS_COLORS[h.status_label] ?? STATUS_COLORS.stable;
              return (
